...
def main(Intcode: list):
    ...
    while (Intcode[0+x] != 99):
        current = opcode(Intcode[0+x], Intcode[1+x], Intcode[2+x], Intcode[3+x])
        if current.what_to_do == 1:
            Intcode[current.what_to_change] = Intcode[current.pos1] + Intcode[current.pos2]
        elif current.what_to_do == 2:
            Intcode[current.what_to_change] = Intcode[current.pos1] * Intcode[current.pos2]
        else:
            pass
